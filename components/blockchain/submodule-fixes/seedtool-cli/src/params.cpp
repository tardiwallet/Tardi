...
static const char * TAG = "Keytool-CLI";
...
void Params::validate_count() {
    ...
    if(count < 1 || count > 1024) {
        ESP_LOGE(TAG, "COUNT must be in [1-1024].");
    }
}
    ...
    if(raw.input_format.empty()) {
        ...
        if(raw.input_format == "ur") {
            ...
            switch(k) {
                ...
                default:
                    ESP_LOGE(TAG, "Unknown input format: %s", raw.input_format.c_str());
                    break;
            }
        }
    }
...
void Params::validate_output_format() {
    if(raw.output_format.empty()) {
        ...
        switch(k) {
            ...
            default:
                ESP_LOGE(TAG, "Unknown output format: %s", raw.output_format.c_str());
                break;
        }
    }
}
...
void Params::validate_output_for_input() {
    ...
    ESP_LOGE(TAG, "Input format %s cannot be used with output format %s",
...
void Params::validate_ints_specific() {
    ...
    if(f != NULL) {
        ...
        if(!(0 <= low && low < high && high <= 255)) {
            ESP_LOGE(TAG, "--low and --high must specify a range in [0-255].");
        }
        f->low = low;
        f->high = high;
    } else {
        if(!raw.ints_low.empty()) {
            ESP_LOGE(TAG, "Option --low can only be used with the \"ints\" output format.");
        }
        if(!raw.ints_high.empty()) {
            ESP_LOGE(TAG, "Option --high can only be used with the \"ints\" output format.");
        }
    }
}
...
void Params::validate_bip39_specific() {
    ...
    if(!FormatBIP39::is_seed_length_valid(count)) {
        ESP_LOGE(TAG, "For BIP39 COUNT must be in [12-32] and divisible by 4.");
    }
}
...
sskr_group_descriptor Params::parse_group_spec(const string &string) {
    ...
    if(items != 2) {
        ESP_LOGE(TAG, "Could not parse group specifier: \"%s\"", string.c_str());
    }
    if(!(0 < threshold && threshold <= count && count <= 16)) {
        ESP_LOGE(TAG, "Invalid group specifier \"%s\": 1 <= N <= M <= 16", string.c_str());
    }
    if(count > 1 && threshold == 1) {
        ESP_LOGE(TAG, "Invalid group specifier. 1-of-M groups where M > 1 are not supported.");
    }
...
void Params::validate_sskr_specific() {
    ...
    if(of == NULL) {
        if(raw_groups_count > 0) {
            ESP_LOGE(TAG, "Option --group can only be used with the \"sskr\" output format.");
        }
        if(!raw.sskr_groups_threshold.empty()) {
            ESP_LOGE(TAG, "Option --group-threshold can only be used with the \"sskr\" output format.");
        }
        return;
    }

    if(!FormatSSKR::is_seed_length_valid(count)) {
        ESP_LOGE(TAG, "For BIP39 COUNT must be in [16-32] and even.");
    }

    vector<sskr_group_descriptor> groups;
    if(raw_groups_count > MAX_GROUPS) {
        ESP_LOGE(TAG, "There must be no more than %d groups.", MAX_GROUPS);
    ...
    if(!(0 < groups_threshold && groups_threshold <= groups.size())) {
        ESP_LOGE(TAG, "Group threshold must be <= the number of groups.");
    }
...
void Params::validate_input() {
    ...
    if (is_random(input_format)) {
        if (!raw.args.empty()) {
            ESP_LOGE(TAG, "Do not provide arguments when using the random (default) input format.");
        }
    } else {
        if (raw.args.empty()) {
        ...
        input = raw.args;
        if(input.empty()) {
            ESP_LOGE(TAG, "No input provided.");
        }
        ...
        if(is_ur_in) {
            ...
            while(i != input.end()) {
                ...
                if(decoder->is_failure()) {
                    ESP_LOGE(TAG, "%s", decoder->result_error().what());
                }
                ...
                if(ur_state == decoding_multi_share) {
                    if(decoder->is_success()) {
                        ...
                        ESP_LOGE(TAG, "Multi-part UR encountered while decoding multi-share.");
                    }
            ...
            if(decoder != NULL) {
                ESP_LOGE(TAG, "Incomplete UR parts.");
            } else {
...
void Params::validate_count_for_input_format() {
    if (is_hex(input_format)) {
        if (!raw.count.empty()) {
            ESP_LOGE(TAG, "The --count option is not available for hex input.");
        }
    } else if (is_bytewords_any(input_format)) {
        if (!raw.count.empty()) {
            ESP_LOGE(TAG, "The --count option is not available for Bytewords input.");
        }
    }
}
...
void Params::validate_ur() {
    ...
    if(raw.is_ur) {
        if(is_ur_in) {
            ESP_LOGE(TAG, "The --ur option may not be combined with the --in ur input method.");
        }
        ...
        if(max_fragment_length < 10) {
            ESP_LOGE(TAG, "MAX_FRAGMENT_LENGTH must be >= 10.");
            return;
        }
        ...
        ESP_LOGE(TAG, "The --ur option is only available for hex, BIP39 and SSKR output.");
    }
}
